...
const apiConfig = window.LibWiseApiConfig;
...
function updateApiLabel() {
    ...
    label.textContent = `Backend API: ${apiConfig.getDisplayBase()}`;
}

function bindApiConfigButton() {
    document.getElementById("apiConfigBtn").addEventListener("click", async () => {
        const current = apiConfig.getBaseUrl();
        const input = window.prompt(
            "Enter backend API base URL (e.g., https://your-backend.onrender.com). Leave blank to auto mode:",
            current
        );
        ...
        try {
            apiConfig.saveBase(input);
            updateApiLabel();
            await loadStats();
            setAdminStatus("API endpoint updated.");
        } catch (error) {
